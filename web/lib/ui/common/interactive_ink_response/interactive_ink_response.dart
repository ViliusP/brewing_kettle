...
///
/// An [InteractiveInkResponse] widget does two things when responding to a tap:
///
...
///    [InteractiveInkResponse]. If it is [BoxShape.rectangle], then the highlight is a box
///    the size of the [InteractiveInkResponse] itself, unless [getRectCallback] is
...
///    grows.
///
/// The following two diagrams show how [InteractiveInkResponse] looks when tapped if the
/// [highlightShape] is [BoxShape.circle] (the default) and [containedInkWell]
/// is false (also the default).
///
/// The first diagram shows how it looks if the [InteractiveInkResponse] is relatively
/// large:
///
...
///
/// The second diagram shows how it looks if the [InteractiveInkResponse] is small:
///
...
///
/// The following diagram shows the effect when the [InteractiveInkResponse] has a
...
/// [DecoratedBox], between the [Material] widget and the [InteractiveInkResponse] widget,
...
/// [Material] above the opaque widget but below the [InteractiveInkResponse] (as an
...
///  * [IconButton], which combines [InteractiveInkResponse] with an [Icon].
class InteractiveInkResponse extends StatelessWidget {
  /// Creates an area of a [Material] that responds to touch.
  ///
  /// Must have an ancestor [Material] widget in which to cause ink reactions.
  const InteractiveInkResponse({
    super.key,
    this.child,
    this.onTap,
    this.onTapDown,
  ...
  ///
  ///
  final WidgetInteractionController? interactionController;
...
class _InkResponseState extends State<_InkResponseStateWidget>
  ...
  void simulateHoldableTap(Duration duration, [Offset? globalPosition]) {
    if (globalPosition != null) {
      _startNewSplash(details: TapDownDetails(globalPosition: globalPosition));
    } else {
      _startNewSplash(context: context);
    }
    _hovering = true;
    handleHoverChange();
    if (duration >= WidgetPress.long().duration && widget.onLongPress != null) {
      Future.delayed(WidgetPress.long().duration, () {
        handleLongPress();
        _hovering = false;
        handleHoverChange();
  ...
  void handleInteractions() {
    if (widget.interactionController?.value == null) return;
    for (var interaction in widget.interactionController!.value) {
      switch (interaction) {
        // case WidgetPress() when interaction.duration < WidgetPress.long().duration:
        //   simulateTap();
        //   break;
        // case WidgetPress() when interaction.duration >= WidgetPress.long().duration:
        //   simulateLongPress();
        ...
        //   break;
        case WidgetPress():
          simulateHoldableTap(interaction.duration, interaction.globalPosition);
          break;
      }
    }
    widget.interactionController?.value.clear();
  }

  void initInteractionController() {
    if (widget.interactionController != null) {
      widget.interactionController?.addListener(handleInteractions);
    }
  }
