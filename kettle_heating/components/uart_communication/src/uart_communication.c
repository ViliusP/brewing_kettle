...
{
    // Buffer to hold the uint32_t as bytes
    uint8_t buffer[4];

    // Convert uint32_t to byte array (big-endian format)
    ...
    // Send the bytes
    const int tx_bytes = uart_write_bytes(UART_NUM_1, (const char *)buffer, sizeof(buffer));
    ESP_LOGI(TAG, "Wrote %d bytes", tx_bytes);
    return tx_bytes;
}
